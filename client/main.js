...
function showSearchPrompt() {
    ...
    for (id in sationDict) {
        ...
        if (searchText.localeCompare(stationNameLower.slice(0, searchText.length)) == 0) {
      ...
      <div class="max-w-xs">
        <div class="bg-white p-2 rounded-lg shadow-lg">
            ...
            <div class="flex flex-col space-y-1">
                <div><span class="font-semibold">Bikes Available:</span> <span class="${d.available_bikes === 0 ? "text-red-600" : "text-gray-600"}">${d.available_bikes}</span></div>
                <div><span class="font-semibold">Stands Available:</span> <span class="text-gray-600">${d.available_stands}</span></div>
            </div>
        </div>
      </div>`;
